...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Add/overwrite context { title }
    await cloudinary.uploader.add_context({ title }, [public_id], { resource_type, type: 'upload' });

    return res.status(200).json({ success: true, public_id, title });
  } catch (err) {
