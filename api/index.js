// api/index.js — Single router for all endpoints (Hobby plan friendly)
// Endpoints:
//   POST   /api/login
//   POST   /api/signup
//   POST   /api/upload
...
//
// Requires helpers alongside this file:
//   /api/_cors.js  (currently wildcard to unblock CORS)
//   /api/_jwt.js
//   /api/_db.js   (optional; this file uses supabase-js directly)
//
// ENV VARS (Vercel → Project → Settings → Environment Variables):
//   AUTH_SECRET
//   SUPABASE_URL, SUPABASE_SERVICE_ROLE
//   CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET, CLOUDINARY_FOLDER
//   ALLOWED_ORIGINS  (used when you flip _cors.js off wildcard)
//   TEACHER_ACCESS_CODE  (optional, default "StEaM4AlL")
...
// Full-name validation (letters incl. Turkish; at least 2 words)
...
async function handleSignup(req, res) {
  ...
  const token = signToken({ name: row.name, role: row.role }); // store exact case as typed
...
async function handleProjects(req, res) {
  ...
  let items = resources.map(r => {
    ...
    return {
      ...
      studentName: c.studentName || null, // exact case
      title: c.title || null,
      makers
    };
  });
...
async function handleHealthOpen(_req, res) {
  // Separate open CORS quick check
...
module.exports = async (req, res) => {
  try {
    // CORS first — currently wildcard via _cors.js while you’re unblocking
    setCORS(req, res);

    // Proper preflight response (some browsers require JSON body)
    ...
    res.statusCode = 404;
    return res.end(JSON.stringify({ success:false, message:"Not found", path:p }));
  } catch (err) {
