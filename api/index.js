// api/index.js — Single router for all endpoints on Vercel Hobby
// Endpoints:
//   POST   /api/login
//   POST   /api/signup
//   POST   /api/upload
...
//
// Uses existing helpers: _cors.js, _jwt.js, _db.js
// Make sure env vars are set: AUTH_SECRET, SUPABASE_URL, SUPABASE_SERVICE_ROLE,
// CLOUDINARY_* , CLOUDINARY_FOLDER, ALLOWED_ORIGINS, (optional) TEACHER_ACCESS_CODE
...
async function getUserByNameRole(name, role) {
  ...
  const { data, error } = await sb.from("users").select("*").eq("role", r).limit(100);
...
async function handleSignup(req, res) {
  ...
  if (!pin) return res.end(JSON.stringify({ success:false, message:"PIN is required." }));
...
async function handleSearchStudents(req, res, url) {
  ...
  const { data, error } = await sb.from("users")
    .select("name").eq("role","student").ilike("name", `%${q}%`).limit(10);
  if (error) throw error;
...
async function handleUpload(req, res) {
  ...
  if (!auth || auth.role !== "student") return res.end(JSON.stringify({ success:false, message:"Please sign in as a student to upload." }));
  ...
  for (const m of makersRequested) {
    ...
    if (row) makersValid.push(row.name); // store exact DB case
  }
  ...
  if (!filepath) return res.end(JSON.stringify({ success:false, message:'No file uploaded (field "projectFile")' }));
...
async function handleRetitle(req, res) {
  ...
  if (!public_id || !title) return res.end(JSON.stringify({ success:false, message:"public_id and title are required" }));
...
async function handleDelete(req, res) {
  ...
  const public_id = String(body.public_id || "").trim();
  const resource_type = body.resource_type === "video" ? "video" : "image";
  if (!public_id) return res.end(JSON.stringify({ success:false, message:"public_id is required" }));
